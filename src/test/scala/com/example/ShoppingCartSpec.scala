...
class ShoppingCartSpec extends AnyFlatSpec with Matchers {
  ...
  """ShoppingCart("cheerios")""" should """ return a shopping cart with Item("Cheerios",8.43)""" in {
    ShoppingCartTest.getShoppingCart(ItemRequest(1,"cheerios")) should be(ShoppingCart(List(CartItem(1, Item("Cheerios", 8.43)))))
  }
  ...
  """ return a shopping cart with CartItem(1,"Cheerios",8.43) and CartItem(1,"Corn Flakes",2.52) """ in {
      ShoppingCartTest.getShoppingCart(ItemRequest(1,"cheerios"),ItemRequest(1,"cornflakes")) should be(ShoppingCart(List(CartItem(1,Item("Cheerios",8.43)),CartItem(1,Item("Corn Flakes",2.52)))))
  }
  ...
  """ShoppingCart("cheerios","cornflakes","bogus) with non-existent "bogus" item """ should
    ...
    ShoppingCartTest.getShoppingCart(ItemRequest(1,"cornflakes"),ItemRequest(1,"cheerios"),ItemRequest(1,"bogus")) should be(ShoppingCart(List(CartItem(1,Item("Corn Flakes",2.52)),
